...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("shows a submitted answer ahead of an older running tool until the agent resumes", () => {
    ...
    const question = {
      ...
      entry: {
        ...
        questionAnswerSubmittedAt: "2026-01-01T00:00:05Z",
      },
    };
    const input = {
      timelineEntries: [
      ...
      latestTurn: { turnId, state: "running" as const, startedAt: createdAt, completedAt: null },
      isWorking: true,
      activeTurnStartedAt: createdAt,
      turnDiffSummaries: [],
      supportsConversationRollback: false,
    };
    expect(deriveMessagesTimelineRows(input).find((row) => row.kind === "work-live")).toMatchObject(
      { entry: { id: "question" }, active: false },
    );
    const { questionAnswerSubmittedAt: _submittedAt, ...resumedQuestion } = question.entry;
    expect(
      deriveMessagesTimelineRows({
        ...input,
